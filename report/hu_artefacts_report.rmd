...
# compare dimensions of retouch and plain flakes
old_names <- colnames(flakes)
flakes_clean <- data.frame(flakes[,1:28], check.names = TRUE)
flakes_clean_summary <- 
flakes_clean %>% 
  mutate(retouch_complete = ifelse(flakes_clean$X.type. == "ret", "Y", "N")) %>% 
  group_by(retouch_complete) %>% 
  summarise_if(is.numeric, mean, na.rm = TRUE)
           
#average max dimension of retouched pieces
retouched_piece <- flakes %>% 
  filter(grepl("ret", type)) 

mean(retouched_piece$`max dimension`, na.rm = TRUE)


# edge shape
retouch$`number of edge`
number_of_edge_shapes <- 
...
edge_checking$diff <- with(edge_checking, count_of_edges - number_of_edge_shapes)
edge_checking$diffl <- ifelse(edge_checking$diff == 0, TRUE, FALSE)

retouch_edge_shapes <- retouch[ edge_checking$diffl, ]
...
retouch_edge_shapes$number_edited <- gsub("f_", "", retouch_edge_shapes$number)
retouch_edge_shapes_metrics <- 
  left_join(retouch_edge_shapes, 
            flakes_clean_names, 
            by = c("number_edited" = "number"))
...
retouch_edge_shapes_metrics %>% 
  ...
  gather_(edge.shape, .dots = list(basic_metrics))
