...
# looking at hist(flakes$`scar number`) I see that most flakes have 3 or fewer scars
# so we can say
flakes_with_3_or_less_scars <- round(sum(flake_scar_numbers <= 3) / length(flake_scar_numbers) * 100, 1)
# what about the relationship between number of flake scars and artefact size?
with(flakes, summary(lm(`scar number` ~ mass)))
with(flakes, plot(`scar number`, mass))
