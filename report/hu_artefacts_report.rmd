...
complete_flakes <- 
flakes %>% 
  filter(!grepl("brk|break|proximal|ret|retouch|pebble", type))
...
xxx_plat_perc <- flakes_platforms[flakes_platforms$platform == "xxx", ]$perc
...
xxx_platshp_perc <- flakes_platform_shape[flakes_platform_shape$`platform shape` == "xx", ]$perc

```


We found `r n_complete_flakes` complete unretouched flakes. The average maximum length of the flakes is `r flake_length_mean` mm, the average thickness is``mm. There are `r sum(flake_platform$n)` flakes or broken flake that have distinguishable platforms, that  can be divided into cortex (`r cortex_plat_perc` %), plain (`r plain_plat_perc` %), facet (%), dihederal (%) and focus (%).  The shapes of platform include triangle (`r triangle_platshp_perc` %), fusiform (%), quadrangle (%) and CDG (%).  Most of flakes dorsal side is partially covered with cortex (%). And the average scar number is xxx. The directions of these scars are recorded.  Among them, the scars with the same directions of flake are dominated (%), following with opposite direction(%). We also found a number of centripetal scars (%).     
