...
# core cortex location 
 cortex_location_percentage <-
   cores %>% 
   mutate(cortext_location_percentage == )
...
# Do levallois pieces have higher angles?
edge_angle <- 
edge_angle %>% 
  gather(variable, value, -number) %>% 
  mutate(value = 2 * ( atan(0.5 * (value/3) ) ) / (pi/180))
...
edge_angle_metrics_lev_var <- 
  edge_angle_metrics_lev %>% 
    ...
    summarise(cv = raster::cv(value, na.rm = TRUE),
...
# ANOVA to test for statistical difference between edge angles and many variables
anova_on_edge_angle_metrics_lev <- 
edge_angle_metrics_lev %>% 
  select(value, type) %>% 
  aov(value ~ type, data = .) %>% 
  tidy()
