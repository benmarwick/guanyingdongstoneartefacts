...
```{r read-in-the-data, eval=TRUE, echo=FALSE}
# read in the data
library(plyr)
library(dplyr)
library(knitr)
...
# file_name <- here::here("analysis/data/data_raw/artefacts-after Sam_Nov2017.xls") 
file_name <- here::here("analysis/data/data_raw/artefacts-notype.xls") 
...
```{r deal-with-duplicates, eval=TRUE, echo=FALSE}
# some mix of upper and lower case
flakes <- flakes %>% 
  mutate_all(tolower)
...
flakes$initiation <-
  ifelse(
  flakes$initiation %in% c("hert", "h"),
  "hertzian",
  ifelse(
  flakes$initiation == "bend",
  "bending",
  ifelse(
  flakes$initiation %in% c("wedging", "wed"),
  "wedge",
  ifelse(
  flakes$initiation %in% c("miss", "mis"),
  "missing",
  flakes$initiation
  )
  )
  ...
  flakes$platform <- ifelse(
  flakes$platform == "plane",
  "plain",
  ifelse(
  flakes$platform %in% c("miss", "mis", "42.000000", "0.000000", "overh"),
  "missing",
  ifelse(flakes$platform == "facted", "faceted", flakes$platform)
  )
  )
  
  flakes$termination <- ifelse(
  flakes$termination == "f",
  "feather",
  ifelse(
  flakes$termination == "br",
  "break",
  ifelse(
  flakes$termination %in% c("brpt", "brput"),
  "abrupt",
  ifelse(
  flakes$termination == "plung",
  "plunge",
  ifelse(
  flakes$termination == "st",
  "step",
  ifelse(
  flakes$termination %in% c("m", "miss", "na", "over"),
  "missing",
  flakes$termination
  )
  )
   ...
   flakes$material <- ifelse(
  flakes$material == "qutz",
  "quartz",flakes$material )

  flakes$`Width_25%max` <- ifelse(flakes$`Width_25%max` == 332.7, 33.2,
  ifelse(flakes$`Width_25%max` == 668, 66.8, flakes$`Width_25%max`))
...
```{r raw-materials, eval = TRUE, rawmaterials}
...
```{r taphnomy, eval = TRUE, taphonomy}
# how many flakes are broken?
flakes_broken <- 
flakes %>% 
  mutate(broken = if_else(grepl("brk|break|proximal", type2), "broken", "not broken")) %>% 
  group_by(broken) %>% 
  tally() %>% 
...
flakes %>% 
  ...
  mutate_at(vars(length            ,  
                 max_dimension     ,   oriented_width   ,    `Width_25%max`       , 
                 `Width_50%max`      ,   `Width_75%max`     ,    oriented_thickness , 
                 `Thickness_25%max`  ,   `Thickness_50%max` ,    `Thickness_75%max`   , 
                 mass                       ,    platform_width     , 
                 platform_thickness, scar_number, cortex_percentage ), as.numeric)

names(flakes)[30] <- "comment"
complete_flakes <- 
flakes %>% 
  filter(!grepl("brk|break|proximal|ret|retouch|pebble|tanged", type1))
n_complete_flakes <- nrow(complete_flakes)

ret_flakes <- 
flakes %>% 
  filter(grepl("ret|retouch", type1))

n_ret_flakes <- nrow(ret_flakes)
flake_length_mean <- round(mean(as.numeric(flakes$length), na.rm = TRUE),1) #there are thousands of flakes
flake_thick_mean <- round(mean(as.numeric(flakes$oriented_thickness), na.rm = TRUE),1)

# dorsal flake scars
# we only want flakes that have 0 or more scars, no NA values
flake_scar_numbers <- as.numeric(flakes$scar_number)[!is.na(as.numeric(flakes$scar_number))]
...
with(flakes, cor(scar_number, mass, use = "pairwise.complete.obs"))


# Flake platforms
flakes_platforms <- 
flakes %>% 
  select(platform, type1) %>% 
  filter(!grepl("NA|0.000000|42.000000|mis", platform)) %>%      
  filter(!grepl("ret|retouch|pebble", type1)) %>% 
  group_by(platform) %>% 
  tally() %>% 
...
facet_plat_perc <- flakes_platforms[flakes_platforms$platform == "faceted?", ]$perc

flakes_platform_shape <- 
flakes %>% 
filter(!grepl("NA", platform_shape)) %>% 
  filter(!grepl("ret|retouch|pebble", type1)) %>% 
...
triangle_platshp_perc <- flakes_platform_shape[flakes_platform_shape$platform_shape == "triangle", ]$perc

fusiform_platshp_perc <- flakes_platform_shape[flakes_platform_shape$platform_shape == "sx", ]$perc

quadrangle_platshp_perc <- flakes_platform_shape[flakes_platform_shape$platform_shape == "sb", ]$perc

CDG_platshp_perc <- flakes_platform_shape[flakes_platform_shape$platform_shape == "CDG", ]$perc
...
flake_cortex_percentage <- flakes$cortex_percentage[!is.na(flakes$cortex_percentage)]
...
new_flakes <- flakes[flakes$platform_shape %in% c("CDG", "triangle", "sb", "sx"), ]
...
ggplot(new_flakes, 
       aes(`Thickness_25%max`, 
           colour = platform_shape)) +
  geom_density()

# dorsal flake cortex
flake_cortex_perc <- flakes$cortex_percentage[!is.na(flakes$cortex_percentage)]
...
flakes_L <-  
  mutate(flakes, L = ifelse(grepl("leva", type1), "L", "N"))
...
thick_plot <- ggplot(flakes_L) +
  geom_density(aes(`Thickness_25%max`), 
               colour = "red", 
               alpha = 0.3,
               size = line_weight) +
  geom_density(aes(`Thickness_50%max`), 
               colour = "green", 
               alpha = 0.3,
               size = line_weight) +
  geom_density(aes(`Thickness_75%max`), 
               colour = "blue", 
               alpha = 0.3, 
               size = line_weight) +
  xlab("Thickness (mm)") +
...
average_platform_thickness <- round(mean(flakes$platform_thickness, na.rm = TRUE),2)
# the average width of platform
average_platform_width <- round(mean(flakes$platform_width, na.rm = TRUE),2)
```
...
flakes %>% 
  ...
  mutate(idx_of_conv = `Thickness_50%max`/`Thickness_75%max`,
         idx_of_flat = `Width_50%max`/`Thickness_50%max`,
         idx_of_elon = length/oriented_width) %>% 
  select(mass, 
         scar_number, 
         idx_of_elon, 
         platform_thickness,
...
cores <- 
  cores %>% 
  mutate_at(vars(mass), as.numeric)
...
# cores shapes
cores %>% 
  select(core_geometry) %>% 
  group_by(core_geometry) %>% 
  tally() %>% 
   mutate(perc = round(n / sum(n) * 100, 1))
# core types 
core_types <- cores %>% 
...
average_max_dimension <- mean(cores$max_dimension, na.rm = TRUE)
...
cortex_percentage<- 
  cores %>% 
  select(cortex_percentage) %>% 
  group_by(cortex_percentage) %>% 
  tally() %>% 
  mutate(perc = round(n / sum(n) * 100, 1))
...
core_cortex_location_percentage <-
  cores %>% 
  mutate(cores_cortex_location = ifelse(cortex_location %in% c("prox", "up"), "platform", 
        ifelse(cortex_location == "distal", "bottom","cortex location"))) %>% 
...
basic_metrics <- c("length" , "max_dimension" , "oriented_width" ,   "Width_25.max" , "Width_50.max" ,   "Width_75.max" , "oriented_thickness" ,   "Thickness_25.max" ,   "Thickness_50.max" ,   "Thickness_75.max" , "mass" ,   "platform_width" , "platform_thickness" ,   "scar_number" , "cortex_percentage")
...
average_max_dim_ret <- flakes_clean %>% 
 filter(grepl("ret|retouch", type1)) %>% 
  dplyr::summarise(mean(max_dimension, na.rm = TRUE))
```
...
retouch_raw_material_metrics %>% 
  filter(grepl("blank|bnk|^ret$|^ret $|retouch|^ret brk$|pebble|chunk", type1)) %>% 
  mutate(type3 = 
  ifelse(type1 %in% c("ret", "ret ", "retouch", "retouched"),
        "flake",
  ifelse(type1 %in% c("ret blank", "ret bnk", "ret brk","retouched blank"), 
        "broken flake", 
  ifelse(type1 == "pebble",
         "pebble", "chunk")))) %>% 
  group_by(type3) %>% 
