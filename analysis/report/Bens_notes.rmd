...
TODO
 - how to measure the difference in spread and peakedness of the thickness distributions? 
...
```{r}
# t-test from YH on blade length and blade core length
# from an excel sheet she sent me with only this
...
apastats::describe.ttest(t_test_output)
```
...
# start with this to set a date for the packages that we use
# from the project root directory
checkpoint::checkpoint("2016-11-30", use.knitr = TRUE, 
                       checkpointLocation = getwd())
# then, so checkpoint can see inside the Rmd...
install.packages("knitr")
# then again, and it will get from MRAN all the pkgs mentioned in the Rmd 
checkpoint::checkpoint("2016-11-30", use.knitr = TRUE)

# make .Rprofile file with this in it:
#### -- checkpoint Autoloader  -- ####
 ...
 # knitr bug, check if knitr is installed, if not, then install
 require("knitr")
...
# add this to .gitignore so we don't track all the check point pkgs
.checkpoint/*

# checking...
# Check that CRAN mirror is set to MRAN snapshot:n
getOption("repos") # yes
# Check that library path is set to ~/.checkpoint:
normalizePath(.libPaths(), winslash = "/") # yes, ok

# travis.yml just runs docker
# dockerfile has MRAN with date hard-coded. 
# This is no good because checkpoint cannot access the Rmd files

## ---------------------------------------------------------------------------

# try packrat

# So back to packrat, keeping MRAN and date in .Rprofile

# install packrat
packrat::init()

# then install the necessary pkgs... then
packrat::snapshot()
