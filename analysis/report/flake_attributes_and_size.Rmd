...
flakes_clustered_cats <- 
  flakes_clustered_cats %>% 
   mutate(cortex_location = if_else(str_detect(cortex_location, "prox"), "proximal", cortex_location)) %>%
...
```{r}
library(gridExtra)
out <- by(data = flakes_clustered_cats_long, 
          INDICES = flakes_clustered_cats_long$variable, 
          FUN = function(m) {
      m <- droplevels(m)
      ...
      m <- m + guides(fill=guide_legend(title=unique(flakes_clustered_cats_long$variable))) 
   })
do.call(grid.arrange, out)

# order of items is
unique(flakes_clustered_cats_long$variable)

# If you want to supply the parameters to grid.arrange
# do.call(grid.arrange, c(out, ncol=3))
```

This plot is more flexible
...
for(i in 1:nrow(categorical_plots)){
  categorical_plots$plot[[i]] <- 
    categorical_plots$plot[[i]] + guides(fill=guide_legend(title=unique(categorical_plots$variable)[i]))
}
...
Some observations on the categorical variables:

- Larger flakes tend not to have any cortex on the platform, and more on the distal end of the flake.
- 
